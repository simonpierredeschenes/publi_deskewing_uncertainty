...
then
  ...
  ros2 launch publi_deskewing_uncertainty cube_main_branch_launch.xml bagfile:=$bag_file use_interpolated_measurements:=true
elif [ $use_interpolated_measurements = "false" ]
then
  ros2 launch publi_deskewing_uncertainty cube_main_branch_launch.xml bagfile:=$bag_file use_interpolated_measurements:=false
else
