...
do
  ...
  then
    ...
    ros2 launch publi_deskewing_uncertainty cube_launch.xml bagfile:=$run_bag final_transformation_file_name:=$matrix_file use_icra_model:=false scale_factor:=$scale_factor use_interpolated_measurements:=true saturation_point:=10.5 &
    sleep 10
