...
if __name__ == "__main__":
    ...
    if len(mapper_matrices) != len(theodolite_matrices):
        print("\033[1;33mWarning: The number of matrices in the mapper file (" + str(len(mapper_matrices)) + ") and theodolite file (" + str(len(theodolite_matrices)) + ") does not match.\033[0m")
    ...
    with open(sys.argv[3], "w") as output_file:
        ...
        for i in range(mapper_matrices.shape[0]):
            ...
            (roll_travelled, pitch_travelled, yaw_travelled) = (abs(roll_theodolite), abs(pitch_theodolite), abs(yaw_theodolite))
            (roll_mapper, pitch_mapper, yaw_mapper) = rotation_matrix_to_rpy(mapper_matrices[i,:3,:3])
            (roll_error, pitch_error, yaw_error) = (abs(roll_theodolite - roll_mapper), abs(pitch_theodolite - pitch_mapper), abs(yaw_theodolite - yaw_mapper))
            ...
            print("Rotation error:\t\troll: " + str(roll_error)[:NUMBER_OF_DIGITS] + "rad,\tpitch: " + str(pitch_error)[:NUMBER_OF_DIGITS] + "rad,\tyaw: " + str(yaw_error)[:NUMBER_OF_DIGITS] + "rad,\t\ttotal: " + str(rotation_error)[:NUMBER_OF_DIGITS] + "rad")
